...
subroutine calculate_mfd_acc(dem,res,area,nx,ny,p)
 ...
 do i=1,nx
  do j=1,ny
   call neighbr_check_mfd(i,j,dem,catchment,positions,nx,ny,npos,p)
  enddo
 enddo
...
recursive subroutine neighbr_check_mfd(i,j,dem,catchment,positions,nx,ny,npos,p)
 ...
 if (catchment(i,j) .le. 0)then
  ...
  do ipos=1,npos
   ...
   if ((inew .lt. 1) .or. (jnew .lt. 1) .or. (inew .gt. nx) .or. (jnew .gt. ny)) cycle
   if (dem(inew,jnew) .gt. dem(i,j))then
    call neighbr_check_mfd(inew,jnew,dem,catchment,positions,nx,ny,npos,p)
    call fract_flow_mfd(i,j,inew,jnew,dem,fract,p,positions,nx,ny,npos)
...
subroutine fract_flow_mfd(iorg,jorg,i,j,dem,fract,p,positions,nx,ny,npos)
 ...
 do ipos=1,npos
  ...
  if (dem(i,j) .gt. dem(inew,jnew))then
   slope = (dem(i,j) - dem(inew,jnew))/30.0 !HACK
   angle_sum = angle_sum + slope
  endif
 enddo
 if (angle_sum .eq. 0.0)then
  fract = 0.0
 else
  slope = (dem(i,j) - dem(iorg,jorg))/30.0 !HACK
